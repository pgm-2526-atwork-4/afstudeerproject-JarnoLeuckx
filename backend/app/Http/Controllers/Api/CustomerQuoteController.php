...
class CustomerQuoteController extends Controller
{
	...
	public function sign(Request $request, ContactRequest $quote): JsonResponse
	{
		...
		if (
			$validated['signature_method'] === 'draw' &&
			!empty($validated['signature_image']) &&
			str_starts_with($validated['signature_image'], 'data:image/png;base64,')
		) {
			$updateData['quote_signature_image'] = $validated['signature_image'];
		} else {
			$updateData['quote_signature_image'] = null;
		}
