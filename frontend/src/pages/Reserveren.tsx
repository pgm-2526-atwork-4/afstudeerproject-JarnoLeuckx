...
type FieldProps = { label: string; type?: string; name?: string; placeholder?: string };
...
export default function ReserverenPage() {
  ...
  const [assistentieType, setAssistentieType] = useState<"" | "luchthaven" | "ziekenhuis">("");
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <form className="space-y-10" onSubmit={(e) => e.preventDefault()}>
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          ...
          <Field label="E-mail" type="email" name="email" placeholder="naam@email.com" />
          <Field label="Telefoonnummer" name="telefoon" placeholder="+32 ..." />
        </div>

        <div>
          <p className="mb-4 text-xs font-semibold text-primary">Vertrek adres</p>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="md:col-span-2">
              <Field label="Straat + nummer" name="straat" placeholder="Straatnaam 12" />
            </div>
            <Field label="Postcode" name="postcode" placeholder="2800" />
            <Field label="Gemeente" name="gemeente" placeholder="Mechelen" />
            <div className="md:col-span-2">
              <Field label="Extra info (optioneel)" name="extra_info" placeholder="Bijv. bel aan bij..." />
            </div>
          </div>
        </div>
        ...
        {/* Assistentie */}
        <div>
          <p className="mb-4 text-xs font-semibold text-primary">Assistentie</p>
          ...
          <div
            className={[
              ...
              assistentie === "ja" ? "max-h-[520px] opacity-100" : "max-h-0 opacity-0 border-transparent",
            ].join(" ")}
          ...
          >
            <div className="p-5">
              ...
              <div className="mt-4 rounded-lg bg-white/70 p-4 text-sm text-gray-800 border border-secondary/20">
                {assistentieType === "luchthaven" ? (
                  <>
                    <span className="font-semibold text-primary">Luchthaven assistentie:</span>{" "}
                    Onze chauffeur helpt u bij het in- en uitstappen, ondersteunt met bagage en begeleidt u waar nodig.
                  </>
                ) : assistentieType === "ziekenhuis" ? (
                  <>
                    <span className="font-semibold text-primary">Afspraak assistentie:</span>{" "}
                    Onze chauffeur kan u begeleiden naar de juiste afdeling en veilig terug naar huis brengen.
                  </>
                ) : (
                  <>
                    <span className="font-semibold text-primary">Tip:</span>{" "}
                    Kies een type assistentie zodat we u zo goed mogelijk kunnen helpen.
                  </>
                )}
              </div>
            </div>
